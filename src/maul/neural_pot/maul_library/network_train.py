...
from maul.neural_pot.maul_library import network_classes, network_test
...
def get_adamw_optimizer(
    ...
    if bayesian_depth is None and bayesian is True:
        log.info(
            "Network is Bayesian for all layers. Attaching weights for mu and rho to AdamW optimizer"
        )
        AdamW = torch.optim.AdamW(
            [
                # H networks
                {
                    "params": [h_network[0].mu_weight, h_network[0].rho_weight][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [h_network[2].mu_weight, h_network[2].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.00001,
                },
                {
                    "params": [h_network[4].mu_weight, h_network[4].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.000001,
                },
                {
                    "params": [h_network[6].mu_weight, h_network[6].rho_weight][
                        start_ind:end_ind
                    ]
                },
                # C networks
                {
                    "params": [c_network[0].mu_weight, c_network[0].rho_weight][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [c_network[2].mu_weight, c_network[2].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.00001,
                },
                {
                    "params": [c_network[4].mu_weight, c_network[4].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.000001,
                },
                {
                    "params": [c_network[6].mu_weight, c_network[6].rho_weight][
                        start_ind:end_ind
                    ]
                },
                # N networks
                {
                    "params": [n_network[0].mu_weight, n_network[0].rho_weight][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [n_network[2].mu_weight, n_network[2].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.00001,
                },
                {
                    "params": [n_network[4].mu_weight, n_network[4].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.000001,
                },
                {
                    "params": [n_network[6].mu_weight, n_network[6].rho_weight][
                        start_ind:end_ind
                    ]
                },
                # O networks
                {
                    "params": [o_network[0].mu_weight, o_network[0].rho_weight][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [o_network[2].mu_weight, o_network[2].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.00001,
                },
                {
                    "params": [o_network[4].mu_weight, o_network[4].rho_weight][
                        start_ind:end_ind
                    ],
                    "weight_decay": 0.000001,
                },
                {
                    "params": [o_network[6].mu_weight, o_network[6].rho_weight][
                        start_ind:end_ind
                    ]
                },
            ],
        ...
        log.info(
            "Some layers are Bayesian. Attaching weights for mu and rho on Bayesian layers to AdamW optimizer"
        )
        optimzable = []
        ...
        if use_ani_wd is False:
            for ith in range(0, len(h_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [
                                h_network[ith].mu_weight,
                                h_network[ith].rho_weight,
                            ][start_ind:end_ind]
                        }
                    )
                else:
            ...
            for ith in range(0, len(c_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [
                                c_network[ith].mu_weight,
                                c_network[ith].rho_weight,
                            ][start_ind:end_ind]
                        }
                    )
                else:
            ...
            for ith in range(0, len(o_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [
                                o_network[ith].mu_weight,
                                o_network[ith].rho_weight,
                            ][start_ind:end_ind]
                        }
                    )
                else:
            ...
            for ith in range(0, len(h_network), 2):
                if ith >= bayesian_depth:
                    ...
                    if ith == 2:
                        optimzable.append(
                            {"params": [h_network[ith].weight], "weight_decay": 0.00001}
                        )
                    elif ith == 4:
                        optimzable.append(
                            {
            ...
            for ith in range(0, len(c_network), 2):
                if ith >= bayesian_depth:
                    ...
                    if ith == 2:
                        optimzable.append(
                            {"params": [c_network[ith].weight], "weight_decay": 0.00001}
                        )
                    elif ith == 4:
                        optimzable.append(
                            {
            ...
            for ith in range(0, len(n_network), 2):
                if ith >= bayesian_depth:
                    ...
                    if ith == 2:
                        optimzable.append(
                            {"params": [n_network[ith].weight], "weight_decay": 0.00001}
                        )
                    elif ith == 4:
                        optimzable.append(
                            {
            ...
            for ith in range(0, len(o_network), 2):
                if ith >= bayesian_depth:
                    ...
                    if ith == 2:
                        optimzable.append(
                            {"params": [o_network[ith].weight], "weight_decay": 0.00001}
                        )
                    elif ith == 4:
                        optimzable.append(
                            {
...
def get_sgd_optimizer(
    ...
    if bayesian_depth is None and bayesian is True:
        log.info(
            "Network is Bayesian for all layers. Attaching bias for mu and rho to SGD optimizer"
        )
        SGD = torch.optim.SGD(
            [
                # H networks
                {
                    "params": [h_network[0].mu_bias, h_network[0].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [h_network[2].mu_bias, h_network[2].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [h_network[4].mu_bias, h_network[4].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [h_network[6].mu_bias, h_network[6].rho_bias][
                        start_ind:end_ind
                    ]
                },
                # C networks
                {
                    "params": [c_network[0].mu_bias, c_network[0].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [c_network[2].mu_bias, c_network[2].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [c_network[4].mu_bias, c_network[4].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [c_network[6].mu_bias, c_network[6].rho_bias][
                        start_ind:end_ind
                    ]
                },
                # N networks
                {
                    "params": [n_network[0].mu_bias, n_network[0].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [n_network[2].mu_bias, n_network[2].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [n_network[4].mu_bias, n_network[4].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [n_network[6].mu_bias, n_network[6].rho_bias][
                        start_ind:end_ind
                    ]
                },
                # O networks
                {
                    "params": [o_network[0].mu_bias, o_network[0].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [o_network[2].mu_bias, o_network[2].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [o_network[4].mu_bias, o_network[4].rho_bias][
                        start_ind:end_ind
                    ]
                },
                {
                    "params": [o_network[6].mu_bias, o_network[6].rho_bias][
                        start_ind:end_ind
                    ]
                },
            ],
        ...
        log.info(
            "Some layers are Bayesian. Attaching bias for mu and rho to SGD optimizer"
        )
        optimzable = []
        ...
        if use_ani_wd is False:
            for ith in range(0, len(h_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [h_network[ith].mu_bias, h_network[ith].rho_bias][
                                start_ind:end_ind
                            ]
                        }
                    )
            ...
            for ith in range(0, len(c_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [c_network[ith].mu_bias, c_network[ith].rho_bias][
                                start_ind:end_ind
                            ]
                        }
                    )
            ...
            for ith in range(0, len(n_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [n_network[ith].mu_bias, n_network[ith].rho_bias][
                                start_ind:end_ind
                            ]
                        }
                    )
            ...
            for ith in range(0, len(o_network), 2):
                if ith >= bayesian_depth:
                    optimzable.append(
                        {
                            "params": [o_network[ith].mu_bias, o_network[ith].rho_bias][
                                start_ind:end_ind
                            ]
                        }
                    )
                ...
                if ith >= bayesian_depth:
                    if ith == 2:
                        optimzable.append(
                            {
                                "params": [
                                    h_network[ith].mu_bias,
                                    h_network[ith].rho_bias,
                                ][start_ind:end_ind],
                                "weight_decay": 0.00001,
                            }
                        )
                ...
                if ith >= bayesian_depth:
                    if ith == 2:
                        optimzable.append(
                            {
                                "params": [
                                    c_network[ith].mu_bias,
                                    c_network[ith].rho_bias,
                                ][start_ind:end_ind],
                                "weight_decay": 0.00001,
                            }
                        )
                ...
                if ith >= bayesian_depth:
                    if ith == 2:
                        optimzable.append(
                            {
                                "params": [
                                    n_network[ith].mu_bias,
                                    n_network[ith].rho_bias,
                                ][start_ind:end_ind],
                                "weight_decay": 0.00001,
                            }
                        )
                ...
                if ith >= bayesian_depth:
                    if ith == 2:
                        optimzable.append(
                            {
                                "params": [
                                    o_network[ith].mu_bias,
                                    o_network[ith].rho_bias,
                                ][start_ind:end_ind],
                                "weight_decay": 0.00001,
                            }
                        )
...
def get_default_ani_optimizers(
    ...
    if ani_class.default_ani is True:
        ...
        log.error(
            f"ERROR - function {inspect.stack()[0][3]} is only for use with default ANI networks of elements"
...
def get_generic_ani_optimizers(
    ani_class: network_classes.ANI,
    learning_rate: float,
    momentum: float = 0.0,
    adam_op: bool = True,
    ...
    log.info(
        f"Getting generic optimizer instances for non-standard ANI. You may be better doing this yourself! "
        f"Check carefully."
    )
    ...
    if adam_op is True:
        ...
        optimizer = torch.optim.SGD(
            ani_class.model.parameters(), lr=learning_rate, momentum=momentum
        )

    else:
        log.error(
            f"ERROR - function {inspect.stack()[0][3]} provide a general AdamW or SGD option both are false "
            f"please pick one."
        )
        return None
...
def validate_bnn(
    ...
    for properties in val_data:
        ...
        true_energies_plot = (
            true_energies_plot + true_energies.clone().detach().tolist()
        )
        predicted_energies_plot = (
            predicted_energies_plot + predicted_energies.clone().detach().tolist()
        )
        predicted_energies_sigma_plot = (
            predicted_energies_sigma_plot
            + predicted_energies_sigma.clone().detach().tolist()
        )
    ...
    log.info(
        "weighted by {} and scaled kl: {}".format(
            kl_weight, scaled_kl.item() * kl_weight
        )
    )
    ...
    if epoch % 1000 == 0:
        ...
        errors = np.array(
            [p - t for p, t in zip(predicted_energies_plot, true_energies_plot)]
        )
        ...
        ax_plt_abs.figure.savefig(
            "uncertainity_against_abs_error_epoch_{}.png".format(epoch)
        )
        ...
        plt.plot(
            ...
            predicted_energies_plot,
            "bo",
            label="True Vs predicted energies (Ha) RMSE: {} Ha {} kcal/mol".format(
                rmse_ha, hartree2kcalmol(rmse_ha)
            ),
        )
        plt.legend()
        ...
        plt.errorbar(
            ...
            capsize=0.1,
            capthick=1.2,
            label="True Vs predicted energies (Ha) with uncert RMSE: {} Ha {} kcal/mol".format(
                rmse_ha, hartree2kcalmol(rmse_ha)
            ),
        )
        plt.legend()
        ...
        log_likelihoods = [
            log_likelihood(p, p_sigma, t)
            for p, p_sigma, t in zip(
                predicted_energies_plot,
                predicted_energies_sigma_plot,
                true_energies_plot,
            )
        ...
        uncertainty_mets = get_uncertainity_metrics(
            np.array(predicted_energies_plot),
...
def validate_dnn(
    ...
    if plot == True:
        ...
        df = pd.DataFrame(
            data=np.array(
                [errors, abs_errors, true_energies_plot, predicted_energies_plot]
            ).T,
            columns=[
                "errors",
                "absolute_errors",
...
def train_dnn(
    ...
    AdamW, SGD = get_default_ani_optimizers(
        ani_class, learning_rate=learning_rate, bayesian_depth=bayesian_depth
    )
    ...
    if (
        ani_class.sgd_ani_scheduler is not None
        and ani_class.adamw_ani_scheduler is not None
        and use_schedulers is True
    ):
        ...
        loss = train_dnn_with_schedulers(
            ani_class,
            train_data,
            val_data,
            test_data,
        ...
        log.info(
            "Note 'use_schedulers' is False will not use Pytorch parameter schedulers"
        )
        loss = train_dnn_without_schedulers(
            ani_class,
            train_data,
            val_data,
            AdamW,
...
def train_dnn_without_schedulers(
    ani_class: network_classes.ANI,
    train_data: torchani.data.TransformableIterable,
    val_data: torchani.data.TransformableIterable,
    AdamW: torch.optim.AdamW,
    ...
    for epoch in range(AdamW_monitor["last_epoch"], max_epochs):
        ...
        log.info(
            "Validation Loss (RMSE): {} at Epoch:{}".format(
                rmse, AdamW_monitor["last_epoch"]
            )
        )
        ...
        if rmse < AdamW_monitor["best"]:
            if ani_class.isparallel is False:
                torch.save(
                    ani_class.model[ani_class._network_layer_indx].state_dict(),
                    best_model_checkpoint,
                )
            else:
                torch.save(
                    ani_class.model.module[ani_class._network_layer_indx].state_dict(),
                    best_model_checkpoint,
                )
            AdamW_monitor["best"] = rmse

        tensorboard.add_scalar("validation_rmse", rmse, AdamW_monitor["last_epoch"])

        tensorboard.add_scalar(
            "best_validation_rmse", AdamW_monitor["best"], AdamW_monitor["last_epoch"]
        )

        tensorboard.add_scalar(
            "learning_rate", learning_rate, AdamW_monitor["last_epoch"]
        )
        ...
        for i, properties in tqdm.tqdm(
            ...
            if ani_class.forces is True:
                coordinates = (
                    properties["coordinates"].to(device).float().requires_grad_(True)
                )
            else:
                coordinates = properties["coordinates"].to(device).float()
            true_energies = properties["energies"].to(device).float()
            ...
            if ani_class.forces is True:
                ...
                log.debug(
                    f"energy loss {energy_loss} force loss {force_loss} force scalar {ani_class.force_scalar} "
                    f"total loss {loss}"
                )
        ...
        if ani_class.isparallel is False:
            ...
            torch.save(
                {
                    "nn": ani_class.model.module[
                        ani_class._network_layer_indx
                    ].state_dict(),
                    "AdamW": AdamW.state_dict(),
                    "SGD": SGD.state_dict(),
                    "epoch": epoch,
        ...
        if epoch == max_epochs - 1:
            ...
            log.info(
                "Validation Loss (RMSE): {} at Epoch:{}".format(
                    rmse, AdamW_scheduler.last_epoch
                )
            )
...
def train_dnn_with_schedulers(
    ...
    for epoch in range(AdamW_scheduler.last_epoch, max_epochs):
        ...
        tensorboard.add_scalar(
            "learning_rate", learning_rate, AdamW_scheduler.last_epoch
        )
        ...
        for i, properties in tqdm.tqdm(
            ...
            if ani_class.forces is True:
                ...
                loss = energy_loss + ani_class.force_scalar * force_loss
                log.debug(
                    f"energy loss {energy_loss} force loss {force_loss} force scaler {ani_class.force_scalar} "
                    f"total loss {loss}"
                )
...
def train_bnn(
    ...
    if (
        ...
        SGD_scheduler = ani_class.sgd_ani_scheduler
        AdamW_scheduler = ani_class.adamw_ani_scheduler
        loss = train_bnn_with_schedulers(
            ani_class,
            train_data,
            val_data,
            AdamW,
        ...
        loss = train_bnn_without_schedulers(
            ani_class,
            train_data,
            val_data,
            AdamW,
...
def train_bnn_without_schedulers(
    ani_class: network_classes.ANI,
    train_data: torchani.data.TransformableIterable,
    val_data: torchani.data.TransformableIterable,
    AdamW,
    ...
    for epoch in range(AdamW_monitor["last_epoch"], max_epochs):
        AdamW_monitor["last_epoch"] = epoch
        rmse_with_mc_runs = validate_bnn(
            ani_class.model,
            val_data,
        ...
        log.info(
            "Validation Loss (RMSE + KL): {} at Epoch:{}".format(
                rmse_with_mc_runs, AdamW_monitor["last_epoch"]
            )
        )
        ...
        tensorboard.add_scalar(
            "validation_rmse", rmse_with_mc_runs, AdamW_monitor["last_epoch"]
        )
        ...
        for i, properties in tqdm.tqdm(
            ...
            if not lml:
                energy_loss = mse_e_loss(
                    predicted_energies, true_energies, num_atoms, mse
                )
            else:
                energy_loss = lml_loss(
                    predicted_energies, true_energies, num_atoms, log=log
                )
            ...
            if ani_class.forces is True:
                ...
                loss = (
                    energy_loss
                    + (ani_class.force_scalar * force_loss)
                    + (kl_weight * scaled_kl)
                )
                log.debug(
                    f"energy loss {energy_loss} force loss {force_loss} force scaler {ani_class.force_scalar} "
                ...
                log.debug(
                    f"energy loss {energy_loss} kl loss {scaled_kl} kl weight {kl_weight} total loss {loss}"
                )
        ...
        if epoch == max_epochs - 1:
            ...
            log.info(
                "Validation Loss (RMSE + KL): {} at Epoch:{}".format(
                    rmse_with_mc_runs, AdamW_scheduler.last_epoch
                )
            )
...
def train_bnn_with_schedulers(
    ...
    for epoch in range(AdamW_scheduler.last_epoch, max_epochs):
        ...
        tensorboard.add_scalar(
            "validation_rmse", rmse_with_mc_runs, AdamW_scheduler.last_epoch
        )
