...
import logging
import torch
from bnnani import network_classes
import torchani
import numpy as np
import pandas as pd
...
def test(ani_class: network_classes.ANI,
    ...
    true_energies, predicted_energies, rmse, pred_energy_sigma = get_energies(ani_class,
                                                                          test_data,
    ...
    if plot is True:
        plot_true_predicted_energies(true_energies,
                                     predicted_energies,
                                     rmse)
        ...
        if ani_class.isbayesian is True:
            errors = np.array([p - t for p, t in zip(predicted_energies, true_energies)])
            ...
            ax_plt_abs = df.plot(x="pred_energy_sigma",
                                 y="absolute_errors",
                                 kind="scatter",
                                 figsize=(15, 15),
                                 grid=True,
            ...
            ax_plt_abs.figure.savefig("{}_uncertainity_against_abs_error.png".format(prepend))

            uncertainty_mets = get_uncertainity_metrics(np.array(predicted_energies),
...
@torch.no_grad()
def get_energies(ani_class: network_classes.ANI,
                 test_data: torchani.data.TransformableIterable,
                 calculate_classification_metrics,
                 mc_runs: int = 2,
                 ):
    ...
    if ani_class.isbayesian is True:
        ...
        for ith, properties in enumerate(test_data.wrapped_iterable):
            ...
            if calculate_classification_metrics is True:
                predictive_uncertainty, model_uncertainty = get_uncertainity_estimation_metrics(
                    torch.stack(mc_predicted_energies))
                log.info(f"Test set predictive uncertainty: {predictive_uncertainty} and model uncertainty: "
                         f"{model_uncertainty}")
...
def plot_true_predicted_energies(true_energies, predicted_energies, rmse):
    ...
    log.info("Converting true and predicted energies from Hartree to kcal/mol for test_predicted_against_known.png")
